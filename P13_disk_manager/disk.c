...
// o disco irá informar isso através de um sinal SIGUSR1 (simulando a
// interrução de hardware que ocorre em um disco real).
//
// O conteúdo do disco simulado é armazenado em um arquivo no sistema
// operacional subjacente, sendo portanto preservado de uma execução para
// outra.
//
...
static disk_t g_disk; // hard disk structure
...
disk_settimer()
{
    ...
    time_ms = abs(g_disk.next_block - g_disk.prev_block)
                  * (g_disk.delay_max - g_disk.delay_min) / g_disk.numblocks
              + g_disk.delay_min
              + random() % (g_disk.delay_max - g_disk.delay_min) / 10;
    ...
    g_disk.delay.it_value.tv_nsec = time_ms * 1000000;

    // primeiro disparo, em segundos
    g_disk.delay.it_value.tv_sec = time_ms / 1000;

    // proximos disparos nao ocorrem (disparo unico)
    g_disk.delay.it_interval.tv_nsec = 0;
    g_disk.delay.it_interval.tv_sec = 0;

    // arma o timer
    if (timer_settime(g_disk.timer, 0, &g_disk.delay, NULL) == -1) {
        perror("DISK:");
        exit(1);
    }
#ifdef DEBUG_DISK
...
disk_sighandle(int sig)
{
    ...
    switch (g_disk.status) {
    case DISK_STATUS_READ:
        // faz a leitura previamente agendada
        lseek(g_disk.fd, g_disk.next_block * g_disk.blocksize, SEEK_SET);
        read(g_disk.fd, g_disk.buffer, g_disk.blocksize);
        break;

    case DISK_STATUS_WRITE:
        // faz a escrita previamente agendada
        lseek(g_disk.fd, g_disk.next_block * g_disk.blocksize, SEEK_SET);
        write(g_disk.fd, g_disk.buffer, g_disk.blocksize);
        break;
    ...
    g_disk.prev_block = g_disk.next_block;

    // disco se torna ocioso novamente
    g_disk.status = DISK_STATUS_IDLE;
...
disk_init()
{
    ...
    if (g_disk.status != DISK_STATUS_UNKNOWN) return -1;

    // estado atual do disco
    g_disk.status = DISK_STATUS_IDLE;
    g_disk.next_block = g_disk.prev_block = 0;

    // abre o arquivo no disco (leitura/escrita, sincrono)
    g_disk.filename = DISK_NAME;
    g_disk.fd = open(g_disk.filename, O_RDWR | O_SYNC);
    if (g_disk.fd < 0) {
    ...
    g_disk.blocksize = DISK_BLOCK_SIZE;
    g_disk.numblocks = lseek(g_disk.fd, 0, SEEK_END) / g_disk.blocksize;

    // ajusta atrasos mínimo e máximo de acesso no disco
    g_disk.delay_min = DISK_DELAY_MIN;
    g_disk.delay_max = DISK_DELAY_MAX;

    // associa SIGIO do timer ao handle apropriado
    g_disk.signal.sa_handler = disk_sighandle;
    sigemptyset(&g_disk.signal.sa_mask);
    g_disk.signal.sa_flags = 0;
    sigaction(SIGIO, &g_disk.signal, 0);

    // cria o timer que simula o tempo de acesso ao disco
    g_disk.sigev.sigev_notify = SIGEV_SIGNAL;
    g_disk.sigev.sigev_signo = SIGIO;
    if (timer_create(CLOCK_REALTIME, &g_disk.sigev, &g_disk.timer) == -1) {
        perror("DISK:");
        exit(1);
    }
...
int
disk_cmd(int cmd, int block, void *buffer)
{
#ifdef DEBUG_DISK
    ...
    case DISK_CMD_STATUS:
        return (g_disk.status);

    // solicita tamanho do disco
    case DISK_CMD_DISKSIZE:
        if (g_disk.status == DISK_STATUS_UNKNOWN) return -1;
        return (g_disk.numblocks);

    // solicita tamanho de bloco
    case DISK_CMD_BLOCKSIZE:
        if (g_disk.status == DISK_STATUS_UNKNOWN) return -1;
        return (g_disk.blocksize);

    // solicita atraso mínimo
    case DISK_CMD_DELAYMIN:
        if (g_disk.status == DISK_STATUS_UNKNOWN) return -1;
        return (g_disk.delay_min);

    // solicita atraso máximo
    case DISK_CMD_DELAYMAX:
        if (g_disk.status == DISK_STATUS_UNKNOWN) return -1;
        return (g_disk.delay_max);

    // solicita operação de leitura ou de escrita
    case DISK_CMD_READ:
    case DISK_CMD_WRITE:
        if (g_disk.status != DISK_STATUS_IDLE) return -1;
        if (!buffer) return -1;
        if (block < 0 || block >= g_disk.numblocks) return -1;

        // registra que ha uma operacao pendente
        g_disk.buffer = buffer;
        g_disk.next_block = block;
        if (cmd == DISK_CMD_READ)
            g_disk.status = DISK_STATUS_READ;
        else
            g_disk.status = DISK_STATUS_WRITE;
